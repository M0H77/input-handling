...
describe('testing isTitle', () => {
    ...
    test('Poop Invalid', () => { expect(v.isTitle("ok💩")).toBe(false) });
...
describe('cleanForHTML and DOM element XSS', () => {
    ...
    test('<script> sanitized', () => { expect(v.cleanForHTML("<script></script>")).toBe("&lt;script&gt;&lt;/script&gt;") })
    test('heart issue', () => { expect(v.cleanForHTML("<3")).toBe("&lt;3") })
    test('<b> and <i> allowed', () => { expectDomChildren(`<b>Bold!</b> and <i>Italics!</i>`, 2) })
